 ...
 */
 ...
 static void SimpleConvexTest(httplib::Client* cli) {
   ...
   httplib::Result res = cli->Post("/GrahamScan", input.dump(), "application/json");
    ...
    for (int it = 0; it < numTries; it++) {
      ...
      if (output["hull_size"] >= 3) {
        for (size_t i = 0; i < output["hull_size"]; i++) {
          ...
          double cross = (b["x"].get<double>() - a["x"].get<double>()) * (c["y"].get<double>() - a["y"].get<double>()) - (b["y"].get<double>() - a["y"].get<double>()) * (c["x"].get<double>() - a["x"].get<double>());
