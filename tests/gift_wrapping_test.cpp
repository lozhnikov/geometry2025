...
bool IsPointInConvexPolygon(const std::vector<std::pair<double,
    ...
    for (size_t i = 0; i < n; i++) {
        ...
        double a = sqrt((polygon[j].second - point.second)*(polygon[j].second - point.second) + (polygon[j].first - point.first)*(polygon[j].first - point.first));
        double b = sqrt((polygon[i].second - point.second)*(polygon[i].second - point.second) + (polygon[i].first - point.first)*(polygon[i].first - point.first));
        double c = sqrt((polygon[j].second - polygon[i].second)*(polygon[j].second - polygon[i].second) + (polygon[j].first - polygon[i].first)*(polygon[j].first - polygon[i].first));
        double p = (a + b + c)/2;
