...
/**
 * @brief Method for Sutherland-Hodgman polygon clipping algorithm implementation.
 * 
...
namespace geometry {
  int SutherlandHodgmanMethod(const nlohmann::json& input, nlohmann::json* output) {
    try {
      ...
      if (!input.contains("subject_polygon") || !input["subject_polygon"].is_array()) {
        (*output)["error"] = "Input must contain 'subject_polygon' array";
        return 1;
      }
      if (!input.contains("clip_polygon") || !input["clip_polygon"].is_array()) {
      ...
      for (const auto& point_json : input["subject_polygon"]) {
        ...
        if (!point_json.contains("y") || !point_json["y"].is_number()) {
